...
Variable store : Type.
Variable stmt : Type.
Variable time_of_stmt : option (N * stmt) -> nat.
Variable empty_store : store.
...
Ltac prog_eq_helper store addr empty_store max Hbounded :=
    let Hmax_le := fresh "H" in
    match goal with
    [s : store, a : addr |- 
        Picinae_timing.program_of_timed_program _ _ 
            (Picinae_timing.timed_program_of_program _ _ _ ?prog ?empty_store ?n)
