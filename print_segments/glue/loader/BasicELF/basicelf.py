from .elfparse import ELFParse, elf_32, elf_64
...
class BasicELF:
    def __init__(self, ELFFile):
        ...
        if self._bits == 32:
            self._structs = elf_32
        else:
            self._structs = elf_64

        #Parse the ELF with the specific structs 
        self.elf = ELFParse(self._structs, self.rawelf)

        # Parse section names
        strtab = self.elf.shdr_table[self.elf.ehdr.e_shstrndx].sh_offset
        for sect in self.elf.shdr_table:
            name = []
            ptr = strtab + sect.sh_name
            while self.rawelf[ptr] != 0:
                name.append(self.rawelf[ptr])
                ptr += 1
            sect.name = bytearray(name)
    ...
    def read(self, vaddr, size):
        ...
        while size:
            for seg in self.elf.phdr_table:
                if vaddr >= seg.p_vaddr and vaddr < seg.p_vaddr + seg.p_memsz:
                    break
            else:
                return None
            off = vaddr - seg.p_vaddr
            if off < seg.p_filesz:
                file_off = seg.p_offset + off
                incr = min(size, seg.p_filesz - off)
                ret += self.rawelf[file_off:file_off + incr]
                vaddr += incr
                size -= incr

            incr = min(size, seg.p_memsz - off);
            ret += b'\0' * incr
            vaddr += incr
            size -= incr
    ...
    def new_segment(self, content=b"", flags=PT_R|PT_W|PT_X, type=PT_LOAD,
            size=None, msize=None, align=0x1000, virtual_address=None,
            physical_off=None, backup_offset=0x0):
        if not self._phdr_fixed:
            self._phdr_fixed = True
            self._fix_phdr(backup_offset)
        ...
        if physical_off is not None and virtual_address is not None:
            ...
            physical_offset = physical_off
            virtual_addr = self._generate_virtual_offset() | (physical_off & 0xfff)
        ...
        print("about to create segment with virtual_addr as ", hex(virtual_addr))
        segment_struct = Container(p_type=type, p_flags=flags,
                p_offset=physical_offset, p_vaddr=virtual_addr,
                p_paddr=virtual_addr, p_filesz=size,
                p_memsz=msize, p_align=align)
        ...
        segment_to_add = Segment(physical_offset, virtual_addr, size,
                flags=flags, align=align, content=content)
 ...
 #    def fix_WSL(self):
    ...
    def _fix_phdr(self, backup_offset):
        ...
        print("in _fix_phdr, physical_offset ", hex(physical_offset))
        print("in _fix_phdr, virtual_addr ", hex(virtual_addr))
        if size >= 0x300:
...
#         for seg in self.elf.phdr_table:
        ...
        new_phdr = self.new_segment(size=0x1000, flags=PT_R|PT_W,
                virtual_address=virtual_addr, physical_off=physical_offset)

        print("small size call")
    ...
    def _find_unused_space(self):
        ...
        for idx in range(len(all_load_segs)-1):
            ...
            print("current segment end poff is ", hex(current_segment_end_poff), "is viable? ", is_viable_for_phdr)
        ...
        # poff is the physical_offset
        print("find unused space, poff is ", hex(poff))
    ...
    def _fix_pdhr_entry(self, offset, virt_addr, size=0x1000):
        print("in _fix_pdhr_entry, argument virt_addr is ", hex(virt_addr))
        ...
        for entry in self.elf.phdr_table:
            if entry.p_type == PT_PHDR:
                ...
                print("we're gonna set ", hex(virt_addr))
    ...
    def _generate_physical_offset(self):
        return max([(len(self.rawelf) & -0x10) + 0x10] + [(s.physical_offset + s.size)
            & -0x10 for s in self._added_segments])

    def _generate_virtual_offset(self):
        PAGE_SIZE = 0x1000

        #Get max virtual address mapped
        tops = [ent.p_vaddr + ent.p_memsz for ent in self.elf.phdr_table 
    ...
    def _update_raw_elf(self):
        ...
        print("in _update_raw_elf, e_phoff is", hex(self.elf.ehdr.e_phoff))
