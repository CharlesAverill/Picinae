...
Module PArch_armv7 := PicinaeStatics ARMArch.
Import PArch_armv7.
...
Definition arm_wtm {s v mem mw} := @models_wtm armtypctx s v mem mw.
Definition arm_regsize {s v u w} := @models_regsize armtypctx s v u w.


(* Create some automated machinery for simplifying symbolic expressions commonly arising
   from arm instruction operations.  Mostly this involves simplifying e mod 2^w whenever
   e < 2^w. *)

(* Define an abbreviation for arm's parity flag computation, which produces uncomfortably
...
Ltac solve_lt_prim :=
  ...
  lazymatch goal with [ M: models armtypctx ?s, R: ?s ?r = Some (VaN ?x ?w) |- ?x < _ ] => apply (arm_regsize M R)
...
Definition arm_subroutine_inv (p:program) inv_set postcond retaddr x (s:store) (n:nat) : option Prop :=
  match x with
...
Remark arm_returning:
  forall ra s n inv_set (postcond: addr -> exit -> store -> nat -> Prop) p m n' s' x',
  postcond ra (Exit ra) s n ->
  next_inv_sat (arm_subroutine_inv p inv_set postcond ra)
               match arm_subroutine_inv p inv_set postcond ra (Exit ra) s n with
               | Some _ => true | None => false end
               (Exit ra) p s m n n' s' x'.
Proof.
  intros. unfold arm_subroutine_inv. destruct (N.eq_dec ra ra) eqn:H'.
...
Remark arm_not_returning_invhere (P: Prop):
  forall p inv_set postcond ra a s n
         (RET: p ra = match p a with Some _ => None | None => Some (0,Nop) end)
         (IS: inv_set a (Exit a) s n = Some P),
  P ->
  match arm_subroutine_inv p inv_set postcond ra (Exit a) s n with Some P => P | None => False end.
Proof.
  intros. unfold arm_subroutine_inv. destruct (N.eq_dec a ra) as [EQ|NE].
...
Remark arm_not_returning_invstep (b:bool):
  ...
  next_inv_sat OP match arm_subroutine_inv p inv_set postcond ra (Exit a) s n with
                  | Some _ => true | None => false end
                  (Exit a) p s m n n' s' x'.
Proof.
  intros.
  unfold arm_subroutine_inv. destruct (N.eq_dec a _) as [EQ|NE].
...
(* Simplify arm memory access permissions between simpl_stmt simplification steps. *)
...
Tactic Notation "arm_simpl_stmt" "in" hyp(H) := simpl_stmt using simpl_memaccs in H.
Tactic Notation "arm_bsimpl" "in" hyp(H) := bsimpl using simpl_memaccs in H.
...
Ltac arm_invhere := first
  [ eapply NISHere, arm_not_returning_invhere; [assumption|reflexivity|simpl_stores]
  | apply arm_returning ]; simpl_arm.

(* Symbolically evaluate an x86 machine instruction for one step, and simplify the resulting
   Coq expressions. *)
Ltac arm_step_and_simplify XS XP' :=
  (arm_bsimpl in XS; [|exact XP']);
...
Ltac arm_invseek :=
  ...
  arm_step_and_simplify XS XP';
  repeat lazymatch goal with [ ACC: MemAcc _ _ _ _ |- _ ] => simpl_arm ACC; revert ACC end; intros;
  ...
  lazymatch type of XS with s=_ /\ x=_ =>
    destruct XS; subst x;
    first [ apply (arm_not_returning_invstep false); [assumption|reflexivity|subst s]
          | apply (arm_not_returning_invstep true); [assumption|reflexivity|subst s]
          | apply arm_returning; subst s ]
  end.
