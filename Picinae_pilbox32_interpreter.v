...
(* Get the 4 bytes of an instruction from memory.
   We use a little endian model, so bits 24-31 are
   retrieved from the greatest address. *)
Definition get_p32_insn (f:addr->N) (a:addr) :=
  cbits (cbits (cbits (f (3+a)) 8 (f (2+a))) 8 (f (1+a))) 8 (f a).
Definition lift_p32 (f:addr->N) (s:store) (a:addr) :=
  Some (4, p322il a (p32_decode (get_p32_insn f a))).
