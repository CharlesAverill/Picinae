...
Theorem wcsnlen_welltyped: welltyped_prog x86typctx wcsnlen_i386.
Proof. Picinae_typecheck. Qed.

(* wcsnlen preserves readable *)
Theorem wcsnlen_preserves_readable:
  forall_endstates wcsnlen_i386 (fun _ s _ s' => s A_READ = s' A_READ).
Proof.
...
Definition wcsnlen_atleast (m:addr->N) (p1:addr) (len:N) :=
  ∀ i, i < len -> 0 <> m Ⓓ[p1 + 4*i].
...
Lemma gt_to_neq (mem:addr->N) (esp:addr) (k:N):
...
Lemma wcsnlen_inductive (m:addr -> N) (p1:addr) (len:N):
...
Lemma wcsnlen_alt_def (m:addr -> N) (p1:addr) (len:N): 
...
Section Invariants.
  ...
  Definition mem' fbytes := mem [Ⓓ esp ⊖ 4 := fbytes ].
  ...
  | 111 | 115 | 122 | 129 => Some (∃ k fb,
      ...
      (wcsnlen_atleast (mem [Ⓓ esp ⊖ 4 := fb ]) (p1) k) /\
...
Theorem wcsnlen_partial_correctness:
  ...
  satisfies_all wcsnlen_i386 (wcsnlen_invs mem esp) wcsnlen_exit ((x',s')::t).
Proof.
  Local Ltac step := time x86_step.
  intros.
  assert (WTM0 := x86_wtm MDL MEM). simpl in WTM0.
  apply prove_invs. simpl. rewrite ENTRY. step. split; assumption.
  ...
  step.
    ...
    step. step. step. exists 1, n. repeat split. 
    unfold wcsnlen_atleast. intros.
    unfold mem'. psimpl.
  ...
  step.
    ...
    step. step. step. eexists 2, n. do 3 split.
    unfold wcsnlen_atleast. intros. 
    ...
    left. unfold mem'; psimpl. eassumption.
  ...
  assert(2 ⊕ k = 1 + (1 + k)). erewrite rewrite_mod_plus. psimpl (2 - 1). reflexivity. 
    psimpl in C1. split. 
    apply Neqb_ok in BC2. subst. change 4 with (2+2) in C1  at 1 . rewrite <- N.add_assoc in C1. apply N.add_cancel_l in C1. 
  ...
  left. 
    ...
    symmetry in C1. rewrite <- N.add_cancel_r with (p:=2). psimpl. rewrite N.add_comm. eassumption.
  (* Prove post-condition for addr 111 (after loop) *)
...
(* step to address 84, fork: continue loop, or return based on ecx =? 1 *)
  ...
  assert(3 ⊕ k = 1 + (2 + k)). erewrite rewrite_mod_plus. psimpl (3 - 1). reflexivity.
    psimpl in C1. split. 
    apply Neqb_ok in BC4. subst. assert (3 + k = p2 mem esp). lia. rewrite H. apply getmem_bound. 
  lia.
  erewrite H. repeat split.
...
(* step to address 99, fork: continue loop, or return based on ecx =? 2 *)
  ...
  assert(4 ⊕ k = 1 + (3 + k)). erewrite rewrite_mod_plus. psimpl (4 - 1). reflexivity.
    psimpl in C1. split. 
    apply Neqb_ok in BC6. subst. assert (4 + k = p2 mem esp). lia. rewrite H. apply getmem_bound. 
  lia.
  erewrite H. repeat split.
