...
Theorem addloop_correctness:
  forall s inp1 inp2 t s' x'
         ...
         (MEM: addloop_mem (s V_MEM32))
         (EXEC: addloop_mem_aexec (s A_EXEC))
         (INP1: s R_R1 = inp1)
         (INP2: s R_R2 = inp2)
,
...
Proof.
  ...
  eapply models_at_invariant; try eassumption. apply welltyped_p32prog. intro MDL1.
  clear - PRE MDL MEM EXEC. rename s into s0, t1 into t, s1 into s.

  destruct_inv 32 PRE.
  destruct PRE as [IN1 [IN2 [MEMSAME EXECSAME]]]. rewrite <-MEMSAME in MEM; rewrite <-EXECSAME in EXEC.
