...
Inductive armvar :=
  ...
  | R8 | R9 | R10 | R11 | R12
  (*SP, the Stack Pointer |LR, the Link Register| PC, the Program Counter*)
  | R13 | R14 | R15
  ...
  | R_J (* (bit 24) is the Java state bit. *)
  | R_Q (* (bit 27) is the sticky overflow bit. *)
  | R_V (* (bit 28) is the overflow bit. *)
  | R_C (* (bit 29) is the carry/borrow/extend bit. *)
  | R_Z (* (bit 30) is the zero bit. *)
  | R_N (* (bit 31) is the negative/less than bit. *)
  | V_TEMP (*********Not sure about this*****).
...
Definition armtypctx (id:varid) : option typ :=
  match id with
  | V_MEM32 => Some (MemT 32)
  | V_MEM64 => Some (MemT 64)
  | R1 | R2 | R3 | R4 | R5 | R6 | R7 => Some (NumT 32)
  | R8 | R9 | R10 | R11 | R12 => Some (NumT 32)
  | R13 | R14 | R15 => Some (NumT 32)
  | R_M => Some (NumT 5)(*(bits 0–4) is the processor mode bits.*)
  | R_T | R_F | R_I | R_A | R_E => Some (NumT 1)
  | R_IT => Some (NumT 8)(* (bits 10–15 and 25–26) is the if-then state bits. *)
  | R_GE => Some (NumT 4)(* (bits 16–19) is the greater-than-or-equal-to bits. *)
  | R_DNM => Some (NumT 4)(* (bits 20–23) is the do not modify bits. *)
  | R_J | R_Q | R_V | R_C | R_Z | R_N => Some (NumT 1)
  | V_TEMP => None
end.
  
Definition x86_wtm {s v mem mw} := @models_wtm armtypctx s v mem mw.
Definition x86_regsize {s v u w} := @models_regsize armtypctx s v u w.
...
Ltac solve_lt_prim :=
  ...
  lazymatch goal with [ M: models armtypctx ?s, R: ?s ?r = Some (VaN ?x ?w) |- ?x < _ ] => apply (x86_regsize M R)
