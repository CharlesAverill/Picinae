(* Picinae: Platform In Coq for INstruction Analysis of Executables       ZZM7DZ
                                                                          $MNDM7
   Copyright (c) 2020 Kevin W. Hamlen            ,,A??=P                 OMMNMZ+
...
(*** SAST DATA STRUCTURE DEFINITIONS ***)
...
Module Type PICINAE_SIMPLIFIER (IL: PICINAE_IL) (TIL: PICINAE_STATICS IL) (FIL: PICINAE_FINTERP IL TIL).
...
(* The metavar tree that interprets metavar identifiers is constructed by treating
   ...
   proof-time, never an incorrect complete proof). *)
...
Fixpoint make_mvtN mvt e {struct e} :=
  ...
  | SIMP_Parity8 e1 => make_mvtN mvt e1
  | SIMP_GetMem _ _ m a
...
Fixpoint make_mvt {A} mvt (t:sastU A) {struct t} :=
  match t with
  | SIMP_RetU _ _ => mvt
  | SIMP_BindN t' e _ => make_mvt (make_mvtN mvt e) t'
  | SIMP_BindM t' e _ => make_mvt (make_mvtM mvt e) t'
  end.
...
(* Simplification is arranged a set of functions, one for each top-level SAST
   ...
   * destruct_matches_def <constr>: same as destruct_matches, except coalesce into a
     ...
     sample code above instead of O(c^2) subgoals (where e1's and e2's types have
     O(c) total constructors). *)
...
(* The following subroutines implement and prove semantic preservation of each
   simplification, organized by SAST constructor.  Modifying and adding to these
   subroutines constitutes the majority of work for improving and extending the
   simplifier.  Most of these subroutines are independent, but the simplification
   for "mod" has a more ambitious implementation that performs a specialized,
   recursive simplification of all operators within the left argument to a "mod";
   and the simplification for bitwise-and calls into the "mod" simplifier whenever
   one of its arguments is the predecessor of a power of two (e.g., when
   simplifying x & (2^y-1) to x mod 2^y. *)
...
Proof.
  intros. unfold simpl_eqb. destruct_matches_def SIMP_NVar; try reflexivity.

  repeat (apply andb_prop in Heqm4; inversion Heqm4 as [? H]; clear Heqm4; rename H into Heqm4). rename Heqm4 into H3.
  apply (sastN_eq_sound mvt) in H0.
  apply sastN_lt_sound in H1. apply sastN_lt_sound in H2. apply sastN_lt_sound in H3.
  cbn [ andb eval_sastB eval_sastN ]. rewrite <- H0.
  rewrite N.eqb_compare. destruct (_ ?= _) eqn:H.

    apply N.compare_eq in H. rewrite H. rewrite N.add_sub, N.mod_same.
      reflexivity.
    ...
    rewrite <- N.add_mod_idemp_l, N.mod_same by apply N.neq_0_lt_0, H1.
    rewrite N.add_0_l, N.mod_small.
      rewrite (proj2 (N.eqb_neq _ _)). reflexivity. apply N.neq_sym, N.sub_gt, N.compare_gt_iff, H.
...
(*** FRONT END: Parsing Coq expressions to Gallina terms ***)
...
(* Opacifying expansion-prone terms:
   ...
   simplification, by copying the existential variable arguments of the metavar
   nodes overtop the other arguments.  Unifying the resulting SAST with the
   original has the secondary function of instantiating all the existential
...
Ltac sastN_gen n :=
  ...
  | (match ?n1 with true => ?n2 | false => ?n3 end) =>
      let t1 := sastB_gen n1 in let t2 := sastN_gen n2 in let t3 := sastN_gen n3 in uconstr:(SIMP_IteB t1 t2 t3)
...
Proof.
  intros.
  Local Ltac checkN e := let t := sastN_gen e in unify (eval_sastN (make_mvtN MVT_Empty t) t) e.
  Local Ltac checkB e := let t := sastB_gen e in unify (eval_sastB (make_mvtB MVT_Empty t) t) e.
  Local Ltac checkM e := let t := sastM_gen e in unify (eval_sastM (make_mvtM MVT_Empty t) t) e.
...
Ltac pos_log2_pow2 p :=
  lazymatch p with
  | xH => uconstr:(N0)
  | xO ?q => let n := pos_log2_pow2 q in uconstr:(N.succ n)
  end.

Ltac populate_var_ids id t :=
  lazymatch t with
...
Tactic Notation "psimpl" uconstr(e) "in" hyp(H) :=
  ...
  (match type of H' with x = ?n =>
     let p := (let t := (let u := sastN_gen n in type_term u) in populate_var_ids N0 t)
     in epose(t1:=p)
   end);
  (let t2 := (eval lazy [t1 simpl_evarsN simpl_evarsB simpl_evarsM] in (simpl_evarsN t1)) in
   let mvt := (eval compute in (make_mvtN MVT_Empty t2)) in
   eapply (simplify_sastN_hyp _ mvt t2) in H';
   [ compute in H'
   | unify t1 t2; reflexivity
   | psimp_verify_frontend ] );
  clear t1;
  cbv beta match delta [noe_setop] in H';
  rewrite H' in H;
  clear x H'.
...
(* Syntax: psimpl_values H
   Description: Simplify all Picinae IL values (VaN/VaM) in hypothesis H. *)

Ltac psimpl_values H :=
  let t1 := fresh "sast" in
  (let t := (let Htyp := type of H in sastV_gen Htyp) in
   (* idtac "SASTV:" t; *) epose (t1:=t));
  (let t2 := (eval lazy [t1 simpl_evarsV simpl_evarsS] in (simpl_evarsV t1)) in
   eapply (simplify_sastV_hyp _ t2) in H;
   [ compute in H
   | unify t1 t2; reflexivity
   | psimp_verify_frontend ] );
  clear t1;
  cbv beta match delta [ noe_typop ] in H;
  (let t0 := fresh "sast" in
   (let t := (let Htyp := type of H in sastU_gen Htyp) in epose (t0:=t));
   let t0def := (eval cbv delta [t0] in t0) in let t := populate_var_ids N0 t0def in
   clear t0; (* idtac "SASTU:" t; *) epose(t1:=t));
  (let t2 := (eval lazy [t1 simpl_evarsU simpl_evarsN simpl_evarsB simpl_evarsM] in (simpl_evarsU t1)) in
   let mvt := (eval compute in (make_mvt MVT_Empty t2)) in
...
Ltac psimpl_goal :=
  let t1 := fresh "sast" in
  (let t := (lazymatch goal with |- ?g => sastV_gen g end) in
   (* idtac "SASTV:" t; *) epose (t1:=t));
  (let t2 := (eval lazy [t1 simpl_evarsV simpl_evarsS] in (simpl_evarsV t1)) in
   ...
   | psimp_verify_frontend ] );
  clear t1;
  cbv beta match delta [ noe_typop ];
  (let t0 := fresh "sast" in
   (let t := (lazymatch goal with |- ?g => sastU_gen g end) in epose (t0:=t));
   let t0def := (eval cbv delta [t0] in t0) in let t := populate_var_ids N0 t0def in
   clear t0; (* idtac "SASTU:" t; *) epose (t1:=t));
  (let t2 := (eval lazy [t1 simpl_evarsU simpl_evarsN simpl_evarsB simpl_evarsM] in (simpl_evarsU t1)) in
   let mvt := (eval compute in (make_mvt MVT_Empty t2)) in
...
End PICINAE_SIMPLIFIER.

Module PicinaeSimplifier (IL: PICINAE_IL) (TIL: PICINAE_STATICS IL) (FIL: PICINAE_FINTERP IL TIL) <: PICINAE_SIMPLIFIER IL TIL FIL.
  Include PICINAE_SIMPLIFIER IL TIL FIL.
End PicinaeSimplifier.



(*

(* The following affords quick stepwise testing of new simplifications for easy
   debugging when modifying the simplifier. *)
...
Import TIL.
Import FIL.
Module M := PicinaeSimplifier IL TIL FIL.
Import M.
...
Proof.
  ...
  match type of H with ?Htyp =>
    let t := sastV_gen Htyp in epose(t1:=t)
  end.
  let t := (eval lazy [t1 simpl_evarsV simpl_evarsS] in (simpl_evarsV t1)) in pose (t2:=t).
  eapply (simplify_sastV_hyp _ t2) in H.
    compute in H.
    2: let t1def := (eval cbv delta [t1] in t1) in let t2def := (eval cbv delta [t2] in t2) in unify t1def t2def; reflexivity.
    2: psimp_verify_frontend.
  clear t1 t2.
  cbv beta match delta [ noe_typop ] in H.

  match type of H with ?Htyp =>
    let t := sastU_gen Htyp in epose(t0:=t)
  end.
  let t0def := (eval cbv delta [t0] in t0) in let p := populate_var_ids N0 t0def in epose (t1:=p).
  clear t0.
  let t := (eval lazy [t1 simpl_evarsU simpl_evarsN simpl_evarsB simpl_evarsM] in (simpl_evarsU t1)) in pose (t2:=t).
  let t := (eval compute in (make_mvt MVT_Empty t2)) in pose (mvt:=t).
  eapply (simplify_sastU_hyp _ mvt t2) in H.
    compute in H.
    2: let t1def := (eval cbv delta [t1] in t1) in let t2def := (eval cbv delta [t2] in t2) in unify t1def t2def; reflexivity.
    2: psimp_verify_frontend.
