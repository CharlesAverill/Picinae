...
(*       (apply N.lt_le_incl; apply N.mod_upper_bound; discriminate). *)
(*   rewrite N.add_comm. *)
(*   rewrite sub_sbop; [|apply N.mod_upper_bound; discriminate]. *)
(*   Search sbop2. *)
(*   rewrite  *)
(*   rewrite <- N.add_sub_assoc by *)
...
Theorem xadd_sub x y
  : x < 2^32 -> y < 2^32 -> x ⊕ (2 ^ 32 + y ⊖ x) = y.
  intros.
  rewrite xminus_alt by assumption.
  rewrite xplus_comm.
  rewrite <- xplus_assoc.
  rewrite (xplus_comm _ x).
  rewrite xminus_zero.
  apply xplus_id_l.
  assumption.
Qed.

Theorem xbits_nested n i1 j1 i2 j2 :
  xbits (xbits n i1 j1) i2 j2 = xbits n (i1 + i2) (N.min j1 (i1 + j2)).
Proof.
  repeat rewrite xbits_equiv.
  rewrite <- (xbits_0_i _ j2), xbits_shiftr, xbits_equiv.
  rewrite (N.add_comm j2 i1), N_mod_mod_pow2_min.
  rewrite N.shiftr_shiftr.
  reflexivity.
Qed.

Theorem xbits_nested_l n i1 j1 i2 j2 :
  j1 <= i1 + j2
  -> xbits (xbits n i1 j1) i2 j2 = xbits n (i1 + i2) j1.
Proof.
  intros.
  replace j1 with (N.min j1 (i1 + j2)) at 2; [apply xbits_nested|].
  apply N.min_l.
  assumption.
Qed.

Theorem xbits_nested_r n i1 j1 i2 j2 :
  i1 + j2 <= j1
  -> xbits (xbits n i1 j1) i2 j2 = xbits n (i1 + i2) (i1 + j2).
Proof.
  intros.
  replace (i1 + j2) with (N.min j1 (i1 + j2)); [apply xbits_nested|].
  apply N.min_r.
  assumption.
Qed.

Theorem bytedup_spec' c :
  xbits (memset_bytedup (c mod 2^8)) 0 8 = c mod 2^8
  /\ xbits (memset_bytedup (c mod 2^8)) 8 16 = c mod 2^8
  /\ xbits (memset_bytedup (c mod 2^8)) 16 24 = c mod 2^8
  /\ xbits (memset_bytedup (c mod 2^8)) 24 32 = c mod 2^8.
Proof.
  ...
  repeat rewrite xbits_0_j.
  repeat rewrite N.lor_0_r.
  repeat ((rewrite xbits_nested_l by discriminate)
          + rewrite xbits_nested_r by discriminate).
  simpl (_+_).
  repeat rewrite xbits_0_i.
  repeat rewrite xbits_none by discriminate.
...
Theorem memset_partial_correctness:
...

  {
    ...
    1-2: rewrite N.add_comm; fold (n _⊖_ n); rewrite (xminus_zero n).
...

  {
    ...
    all: rewrite Hsv,Hsv0,Hsv1,Hsv2,H in *.
    {
      intuition; [rewrite <- xplus_assoc,xadd_sub; assumption + reflexivity|].
      rewrite xplus_assoc.
    ...
    }
    {
      ...
      replace (N.pos (2^8)) with (2^8); [|reflexivity].
      intuition.
      {
        unfold memset_bytedup.
      ...
      {
        replace (2^2) with 4; auto.
        destruct (n0 mod 4); [reflexivity|discriminate].
      }
    }
  }

  {
    ...
    1: repeat rewrite xplusinv by (reflexivity + apply N.mod_upper_bound; discriminate).
    1: intuition.
    {
      rewrite N_mod_mod_pow2_min.
      rewrite N.add_mod, N.add_0_r, N.mod_mod by discriminate.
      exact H0.
    }
    {
      rewrite H2.
      ...
      simpl (8+_).
      rewrite xadd_sub; assumption + reflexivity.
    }
    ...
    all: repeat rewrite set_dword_aligned.
    all: try assumption.
    all: try (apply N.mod_upper_bound; discriminate).
