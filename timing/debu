...
Proof using.
    ...
    do 2 eexists. repeat split. unfold msub. now psimpl.
        rewrite getmem_noverlap; cycle 1.
            apply noverlap_shrink with (a1 := gp) (len1 := __global_size), noverlap_symmetry, 
                noverlap_shrink with (a1 := sp) (len1 := vTaskSwitchContext_stack_frame_size); 
                cycle 1.
            psimpl.
        (* unfold pxCurrentTCB. Compute base_sp. unfold msub; psimpl. 
        repeat rewrite getmem_nbyte_distance.
        all: unfold msub; psimpl; try lia. *)
        admit. admit.
