...
Module Program_ChaCha20 <: ProgramInformation.
    Definition entry_addr : N := 0x10c00.

    Definition exits (t:trace) : bool :=
        match t with (Addr a,_)::_ => match a with
        | 0x10ce4 | 0x10ce8 => true
        | _ => false
    end | _ => false end.
...
Section Invariants.
  ...
  Variable plaintext : addr     (* chacha20_encrypt: 1st pointer arg (R_A0) *).
  Variable ciphertext : addr    (* chacha20_encrypt: 2nd pointer arg (R_A1) *).
  Variable len : N              (* chacha20_encrypt: 3rd arg (R_A2) *).
  Variable key : addr           (* chacha20_encrypt: 4th arg (R_A3) *).
  Variable nonce : addr         (* chacha20_encrypt: 5th arg (R_A4) *).
  Variable counter : addr       (* chacha20_encrypt: 6th arg (R_A5) *).

  Definition time_of_chacha20_block : N := 0.

  Definition time_of_chacha20_encrypt (s:store) : N :=
    if len =? 0 then ttbeq + tjalr else
      tfbeq + taddi + (10 * tsw) + (8 * taddi) + tjal + (6 * taddi).
      (* time_of_chacha20_block +
      tsub +
      if (0 >=? ) *)
