...
Definition non_overlapping_smem_layout (gp : N) (mem : memory) : list (N * N) :=
    [
        (gp ⊖ 920 ⊕ (31 ⊖ clz (uxTopReadyPriority gp mem) 32) * 20, 4);
        (gp ⊖ 1952, 4); (* uxSchedulerSuspended *)
        (gp ⊖ 1932, 4); (* xYieldPendings*)
        (gp ⊖ 1896, 4); (* pxCurrentTCB *)
        (gp ⊖ 1920, 4) (* uxTopReadyPriority *)
    ].
Definition smem_well_formed (gp : N) (mem : memory) : Prop :=
    forall x xw y yw,
        x <> y ->
        In (x, xw) (non_overlapping_smem_layout gp mem) ->
        In (y, yw) (non_overlapping_smem_layout gp mem) ->
        ~ overlap 32 x xw y yw.

Definition non_overlapping_mem_layout (mem : memory) (gp sp : N) : list (N * N) :=
    [
        (gp ⊖ __global_size, __global_size);
        (sp ⊖ vTaskSwitchContext_stack_frame_size, vTaskSwitchContext_stack_frame_size);
        (48 + pxCurrentTCB gp mem, 4);
        (mem Ⓓ[48 + pxCurrentTCB gp mem], 4);
        (8 + mem Ⓓ[48 + pxCurrentTCB gp mem], 4);
        (12 + mem Ⓓ[48 + pxCurrentTCB gp mem], 4);
        (4 + mem Ⓓ[gp ⊖ 920 ⊕ (31 ⊖ clz (uxTopReadyPriority gp mem) 32) * 20], 4);
        (4 + mem Ⓓ[ 4 + mem Ⓓ[gp ⊖ 920 ⊕ (31 ⊖ clz (uxTopReadyPriority gp mem) 32) * 20] ], 4)
    ].
Definition memory_well_formed (gp sp : N) (mem : memory) :=
    smem_well_formed gp mem /\
        forall x xw y yw,
            x <> y ->
            In (x, xw) (non_overlapping_mem_layout mem gp sp) ->
            In (y, yw) (non_overlapping_mem_layout mem gp sp) ->
            ~ overlap 32 x xw y yw.
...
Definition vTaskSwitchContext_timing_invs (p : addr) 
    (base_sp gp : N) (base_mem : memory) (t:trace) : option Prop :=
match t with (Addr a, s) :: t' =>
    let mem := s V_MEM32 in
    match a with
    ...
    *)
    ...
    (* vTaskSwitchContext *)
    | 0x80001380 => Some (
        s R_GP = gp /\ s R_SP = base_sp 
            /\ s R_A4 = uxSchedulerSuspended gp base_mem /\
            memory_well_formed gp base_sp mem /\
...
Theorem vTaskSwitchContext_timing:
  forall s p t s' x' gp sp mem
         ...
         (SP : s R_SP = sp)
         (MEM : s V_MEM32 = mem)
         (NOVERLAPS : memory_well_formed gp sp mem),
  satisfies_all 
    lifted_vTaskSwitchContext                                 (* Provide lifted code *)
    (vTaskSwitchContext_timing_invs p sp gp mem)            (* Provide invariant set *)
...
Proof using.
    ...
    Local Ltac unfold_mem_wf :=
        match goal with
        | [H: memory_well_formed _ _ _ |- _] =>
            unfold memory_well_formed in H;
            destruct H as [SMEM_WF NOVERLAPS]
        end.
    unfold_mem_wf.
    ...
    (* 0x800015e4 *)
    destruct PRE as (GP & SP & A4 & MWF & PreservesPriority & Preserves920).
    unfold_mem_wf.
    repeat step. handle_ex.
    {
        unfold smem_well_formed. intros.
        unfold non_overlapping_smem_layout, uxTopReadyPriority in H0, H1.
        rewrite getmem_noverlap in H0, H1.
        rewrite getmem_noverlap in H0, H1.
        rewrite getmem_noverlap in H0, H1.
        simpl in H0, H1.

        destruct H0. inversion H0; subst x xw.
            destruct H1. inversion H1; subst y yw. contradiction.
            destruct H1. inversion H1; subst y yw.
            - apply SMEM_WF. psimpl.
        admit.
    }
