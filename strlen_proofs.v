...
(* CATEGORY: modular arithmetic lemmas *)
Lemma Nsub_distr:
...
(* CATEGORY: additional modular arithmetic lemma *)
...
Fixpoint coqloop3 st addr steps : option N :=
  match st addr,steps with
  | 0,_ => Some 0
  | _,0%nat => None
  | _,S n' => option_map N.succ (coqloop3 st (N.succ addr) n')
  end.

Fixpoint coqloop2 st addr steps : option N :=
  let x := st Ⓓ[addr] in
  match x ⊕ 4278124287 .& 16843008,steps with
  | 0,_ => Some 0
  | _,0%nat => None
  | _,S n' => coqloop2 st (addr ⊕ 4) n'
  end.

Fixpoint coqloop1 st addr steps : option N :=
  match addr mod 4,st addr,steps with
  | 0,_,_ => coqloop2 st addr steps
  | _,0,_ => Some 0
  | _,_,0%nat => None
  | _,_,S n' => option_map N.succ (coqloop1 st (N.succ addr) n')
  end.
